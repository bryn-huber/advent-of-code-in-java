...
public class Day05 {
  ...
  public Day05() throws IOException {
    List<String> lines = Files.readAllLines(Paths.get("src/main/resources/day05_data.txt"));

    parse(lines);

    LOGGER.log(INFO, "Part 1, lowest location: " + getLowestLocationSingleSeeds());

    parse(lines);

    LOGGER.log(INFO, "Part 2, lowest location: " + getLowestLocationSeedRanges());
  }
      ...
      for (long seed = start; seed < end; seed++) {
        long location = humidityToLocation.getDestination(temperatureToHumidity.getDestination(
            lightToTemperature.getDestination(
                waterToLight.getDestination(fertilizerToWater.getDestination(
                    soilToFertilizer.getDestination(seedToSoil.getDestination(seed)))))));
  ...
  private void parse(List<String> lines) {
    ...
    for (String line : lines) {
      if (line.trim().isEmpty()) {
        ...
        currentMap.addRange(Long.parseLong(parts[0]), Long.parseLong(parts[1]),
